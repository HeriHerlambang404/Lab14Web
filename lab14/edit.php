...
if (isset($_POST['update'])) {
    ...
    if ($nama_gambar != "") {
        ...
        move_uploaded_file($tmp_name, 'uploads/' . $file_baru);
        $path_gambar = 'uploads/' . $file_baru;

        // Hapus file gambar lama dari folder agar tidak menumpuk
        $data_lama = mysqli_query($conn, "SELECT gambar_url FROM produk WHERE id='$id'");
        $row_lama = mysqli_fetch_assoc($data_lama);
        if(!empty($row_lama['gambar_url']) && file_exists($row_lama['gambar_url'])) {
            unlink($row_lama['gambar_url']);
        }

        $query = "UPDATE produk SET nama_barang='$nama', kategori='$kategori', harga_beli='$harga_beli', harga_jual='$harga_jual', stok='$stok', gambar_url='$path_gambar' WHERE id='$id'";
    } else {
        // Jika tidak ganti gambar, update data lainnya saja
        $query = "UPDATE produk SET nama_barang='$nama', kategori='$kategori', harga_beli='$harga_beli', harga_jual='$harga_jual', stok='$stok' WHERE id='$id'";
    }
...
$id = $_GET['id'];
$ambildata = mysqli_query($conn, "SELECT * FROM produk WHERE id='$id'");
...
<head>
    <meta charset="UTF-8">
    <title>Edit Barang - My Inventory</title>
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="harga_beli" class="form-control" value="<?php echo $row['harga_beli']; ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <?php if($row['gambar_url']): ?>
                                    <img src="<?php echo $row['gambar_url']; ?>" class="preview-img">
                                <?php else: ?>
